...
function parseXCI(path) {
	const rom_stream = fs.openSync(path, 'r'); // Open the stream
	...
	fs.readSync(rom_stream, cart_type, 0, 0x1, 0x10D);
	...
	fs.readSync(rom_stream, root_buffer, 0, 0x8, 0x130);
	const FS_OFFSET = struct.unpack('<Q', root_buffer)[0].toNumber();

	metadata.partitions.root = parsePartition(rom_stream, FS_OFFSET);
	...
	for (const partition of metadata.partitions.root.files) {
		...
		metadata.partitions[partition.name] = parsePartition(rom_stream, partition_offset);
	}
...
function parseNCA(path_or_buff) {
	/*
		...
		I'm not even sure I'm getting the tweak right, and no actual decryption is going on
	*/
	...
	function decryptHeader() {
		const header = buffer.subarray(0, 0xC00);
		return decrypt_xts(header, 0x400, 0x200);
	}
	
	function decrypt_xts(source, length, sector_size, current_sector=0) {
		for (let i = 0; i < length; i += sector_size) {
			const tweak = get_tweak(current_sector++);
			const decipher = crypto.createDecipheriv('aes-256-xts', KEYS.header_key, tweak);
			decipher.update(buffer.subarray(0, 0xC00));
			
			return decipher.final();
		}
	}

	metadata.decrypted.header = decryptHeader();
