...
function parseStart(nro_stream) {
	...
	return {
		MOD0_offset: start.subarray(0x04, 0x08).readUInt32LE()
	};
}
...
function parseHeader(nro_stream) {
	...
	const metadata = {
		magic: header.subarray(0, 0x04).toString(),
		format_version: header.subarray(0x04, 0x08).readUInt32LE(),
		size: header.subarray(0x08, 0x0C).readUInt32LE(),
		flags: header.subarray(0x0C, 0x10),
		segment_headers: [
			segmentHeader(header.subarray(0x10, 0x18)),
			segmentHeader(header.subarray(0x18, 0x20)),
			segmentHeader(header.subarray(0x20, 0x28))
		],
		bss_size: header.subarray(0x28, 0x2C),
		build_id: header.subarray(0x30, 0x50)
	};
...
function segmentHeader(buffer) {
	return {
		file_offset: buffer.subarray(0x00, 0x04).readUInt32LE(),
		size: buffer.subarray(0x04, 0x08).readUInt32LE()
	};
}
...
function getHomebrewMetadata(nro_stream) {
	...
	const metadata = {
		magic: asset_header.subarray(0, 0x04).toString(),
		format_version: asset_header.subarray(0x04, 0x08).readUInt32LE(),
		asset_section_headers: {
			icon: assetSectionHeader(asset_header.subarray(0x08, 0x18)),
			nacp: assetSectionHeader(asset_header.subarray(0x18, 0x28)),
			romfs: assetSectionHeader(asset_header.subarray(0x28, 0x38))
		}
	};
...
function assetSectionHeader(buffer) {
	return {
		file_offset: readUInt64LE(buffer.subarray(0x00, 0x08)),
		size: readUInt64LE(buffer.subarray(0x08, 0x10))
	};
}
